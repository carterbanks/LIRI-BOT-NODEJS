...
function myTweets() {
    ...
    client.get('statuses/home_timeline', function (error, tweets, response) {
        ...
        for (let i = 0; i < tweets.length; i++) {
            ...
            logMessage(`On ${tweetDate.format('M-D-YY')}, at ${tweetDate.format('hh:mm A')} CBanks said "${tweetText}."`);
        }
    });
}
...
function spotifyThisSong(songQuery) {
    ...
    spotify.search({ type: 'track,artist', query: songQuery, limit: 1 }, function (err, data) {
        ...
        let message = `This might be what you're looking for...\n` +
            `Track Title:\t${artist}\n` +
            `Artist Name:\t${title}\n` +
            `Album Name:\t${album}\n` +
            `Preview:\t${(preview || "Preview is not available.")}\n`
        logMessage(message);
    });
}
